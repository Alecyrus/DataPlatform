...
<script>
  export default {
    ...
    mounted() {
      ...
      this.searchKeyword = {
          filter:{
              "publickey.$binary":this.cert.publickey.$binary
          }
      }
      ...
      randomFrom(lowerValue,upperValue)
{
    return Math.floor(Math.random() * (upperValue - lowerValue + 1) + lowerValue);
      ...
      search() {
        ...
        this.$Loading.start();
        this.results = [];
        this.$request.post('/api/v1/search?&limit=' + this.requestLimit + "&skip=" + this.skip, this.searchKeyword)
          .then((response) => {
