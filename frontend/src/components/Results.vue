    ...
    <section>
      ...
      <div class="desc">
        ...
        <Row type="flex" align="top" justify="start" style="margin-top:1em;">
          ...
          <transition name="fadeUp" appear>
            <Col span="3" align="center" offset="0">
          ...
          <transition name="fadeUp" appear>
            ...
            <Row type="flex" align="top" justify="start" style="margin-top:0em;">
              <Col span="6" align="left" style="margin-top:4em;margin-bottom:2em" offset="0">
                ...
                <transition-group name="fadeUp" tag="div" appear>
                  ...
                  <Card class="cert" :padding="0" v-for="(item, index) in results" :key="index" style="margin-bottom:1em">
                    ...
                    <Row type="flex" justify="start" align="middle" style="margin-top:0.5em;margin-left:0.8em;">
                      ...
                      <p @click="enterCertDetail(item, index)" class="itemm" style="">{{ getTitle(item) }}</p>
                    ...
                    <Row v-if="hassan(item)" type="flex" justify="start" align="middle" style="margin-top:0.1em;margin-left:0.8em;">
                      ...
                      <Col span="18" align="left" style="margin-left:0.5em;" offset="0">
                      <div style="width:100px;">
                        <p style="font-size:1em;white-space:nowrap;word-wrap:break-word;word-break:break-all;">{{ item.extensions.subjectalternativename.dnsnames.join(",").substr(0,50)}}</p>
          ...
          <transition name="fadeUp" appear>
            ...
            <Col span="6" align="center">
              ...
              <Input v-model="searchKeyword" @keyup.enter="search" autocomplete="on" :spellcheck="true" size="large" style="box-shadow: 0px 4px 16px 0px rgba(0, 0, 0, .1), 0px 0px 8px 0px rgba(0, 0, 0, .02);"
                :placeholder="searchKeyword"></Input>


              </Col>
              <Col span="7" align="left" style="margin-top:2em;" offset="1">

              <Button type="ghost" style="opacity:0.9;box-shadow: 0px 4px 16px 0px rgba(0, 0, 0, .1), 0px 0px 8px 0px rgba(0, 0, 0, .02);"
                class="shadow" shape="circle" icon="ios-search" @click="search"> Search</Button>
                      ...
                      <div style="text-align:center">
                        ...
                        <Row type="flex" align="top" justify="center" style="margin-top:0em;">
                          ...
                          <p style="font-size:1.2em">
                            <Icon type="ios-pricetags-outline"></Icon> 关联数据
                            <strong> {{total}} </strong>条</p>
                          </Col>
                        </Row>
     ...
     -->
                        ...
                        <Row type="flex" align="top" justify="center" style="margin-top:0em;">
                          <Col span="22" align="center" style="margin-top:1em;" offset="1">
                          <p style="font-size:1.2em">
                            <Icon type="android-time"></Icon> 检索用时
                            <strong> {{randomFrom(1,100)/1000}} </strong>s</p>
                          </Col>
                        </Row>
  ...
  export default {
    ...
    methods: {
      ...
      search() {
        ...
        this.$Loading.start();
        this.results = [];
        this.$request.post('/api/v1/search?keywords=' + this.searchKeyword + "&limit=" + this.requestLimit + "&skip=" +
            this.skip)
          .then((response) => {
            let state = response.data.state;
            if (state) {
              this.$toast.success('检索到' + response.data.total + "条数据", 'Info', {
                position: "topRight"
              });
              this.$Loading.finish();
              this.requestIndex++;
              ...
              this.$toast.error('未检索到相关数据!', 'Info', {
                position: "topCenter"
              });
              this.$Loading.finish();
            }
      ...
      handleReachBottom() {
        ...
        return new Promise(resolve => {
          setTimeout(() => {
            ...
            this.$Loading.start();
            this.$request.post('/api/v1/search?keywords=' + this.searchKeyword + "&limit=" + this.requestLimit +
                "&skip=" + this.skip)
              .then((response) => {
                let state = response.data.state;
                if (state) {
                  ...
                  this.$toast.error('未检索到更多数据!', 'Info', {
                    position: "topCenter"
                  });
                  this.$Loading.finish();
                }
...
<style scoped lang="scss">
  ...
  .titlewords {
    ...
    font-size: 3.2em;
    user-select: none;
    font-weight: bolder; // text-shadow: black 0.1em 0.1em 1.5em;
  }
